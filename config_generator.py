...
parser.add_argument("-d", "--is_a_directory", action="store_true", help="Whether the model is in Huggingface format. Default False")
...
for name, value in model.named_modules():
    if type_of_t5_module(name, value) is not None:
...
for i in range(CONF_NUMBER):
    ...
    for key in module_dict:
        if type_of_t5_module(key, module_dict[key]) == "MultiheadAttention":
            conf_i[key] = args.attention[i]
        elif type_of_t5_module(key, module_dict[key]) == "Linear":
